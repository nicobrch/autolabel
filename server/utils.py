...
class InferenceAPI:
    ...
    def segment_object(self, object_id: int, frame_idx: int, db: Session = next(get_db())) -> np.ndarray:
        """
        Segment an object using SAM2 and save the mask to the database.

        Args:
            object_id: ID of the object to segment
            frame_idx: Frame index to segment on
            db: Database session
        ...
        if not current_frame:
            raise ValueError(
                f"Frame with ID {frame_idx} not found in database.")
        ...
        logger.info(
            f"Performing segmentation for object {object_id} on frame {frame_idx}")
        ...
        if existing_mask:
            ...
            logger.info(
                f"Updated existing mask for object {object_id}, frame {frame_idx}")
        else:
            new_mask = Mask(
                object_id=object_id,
            ...
            db.add(new_mask)
            logger.info(
                f"Created new mask for object {object_id}, frame {frame_idx}")
