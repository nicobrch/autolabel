...
# Endpoint to download the YOLO segmentation dataset ZIP file
@app.get("/api/v1/videos/{video_id}/download-yolo-seg-dataset", response_class=FileResponse)
async def download_yolo_seg_dataset(video_id: int, db: Session = Depends(get_db)):
    """
    Generate and download a YOLO segmentation dataset ZIP file for the specified video.
    """
    # Check if video exists
    video = db.query(Video).filter(Video.id == video_id).first()
    if not video:
    ...
    try:
        # Generate the YOLO segmentation dataset ZIP file
        from utils import create_yolo_seg_dataset_zip
        zip_path = create_yolo_seg_dataset_zip(video_id, db)

        if not zip_path or not os.path.exists(zip_path):
            raise HTTPException(
                status_code=500,
                detail="Failed to generate YOLO segmentation dataset"
            )

        # Return the file as a download with appropriate filename
        return FileResponse(
            path=zip_path,
            filename=f"{video_name}_yolo_seg_dataset.zip",
            media_type="application/zip"
        )

    except Exception as e:
        logger.error(f"Failed to generate YOLO segmentation dataset: {str(e)}")
        raise HTTPException(
            status_code=500,
            detail=f"Failed to generate YOLO segmentation dataset: {str(e)}"
        )
